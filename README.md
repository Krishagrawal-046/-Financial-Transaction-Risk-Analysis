...
Transaction Summary: Counts total transactions, calculates average and max amounts.
Top Largest Transactions: Lists the 10 highest-value transactions with details.
Balance Mismatches: Finds transactions where balances don't update as expected (e.g., old - amount ≠ new).
Zero-Balance Mule Accounts: Identifies recipients with frequent zero balances (potential fraud conduits).
Large Transfers/Cash-Outs: Filters transfers or cash-outs exceeding $200,000.
High-Frequency Transfers: Detects accounts making multiple transfers in the same time step (structuring indicator).
Round-Trip Laundering: Spots pairs of accounts exchanging funds back and forth.
Structuring Patterns: Flags senders distributing identical amounts to multiple recipients.
Collector Accounts (Top 10): Recipients receiving from many unique senders.
Distributor Accounts (Top 10): Senders transferring to many unique recipients.
Chain Laundering: Tracks multi-step flows (e.g., A → B → C) in the same step.
Mule-Related Transactions: All activity involving flagged mule accounts.
Volume by Transaction Type: Aggregates counts and totals per type.
Daily Suspicious Amounts: Sums risky amounts per step (e.g., large or mule-involved).
Dual-Role Accounts: Accounts that both send and receive, often intermediaries in laundering.
